  ...
  - Returns "Error: Missing Supabase configuration" if env vars not set`,
    ...
    async ({ response_format }) => {
      try {
        ...
        const totalCost = (aggregateRows ?? []).reduce((s, c) => s + ((c.cost_usd as number) || 0), 0);
        const withDuration = (aggregateRows ?? []).filter((c) => ((c.duration_seconds as number) || 0) > 0);
        const avgDuration = withDuration.length > 0
          ? Math.round(withDuration.reduce((s, c) => s + ((c.duration_seconds as number) || 0), 0) / withDuration.length)
          : 0;
    ...
    async ({ response_format }) => {
      try {
        ...
        const agentMap = new Map<string, {
          total_calls: number; analyzed: number; calls_with_errors: number;
          total_errors: number; critical_errors: number;
          errorCounts: Map<string, number>;
        }>();

        for (const call of calls ?? []) {
          const name = (call.client_name as string) || 'Unknown';
          ...
          if (call.analysis_status === 'complete') {
            a.analyzed++;
            const ec = (call.error_count as number) ?? 0;
            const cc = (call.critical_error_count as number) ?? 0;
            if (ec > 0) a.calls_with_errors++;
            a.total_errors += ec;
            a.critical_errors += cc;
            const analysis = call.call_errors as { errors?: Array<{ type: string }> } | null;
        ...
        if (response_format === 'json') {
          ...
          for (const a of summary) {
            ...
            if (a.top_errors.length > 0) lines.push(`- Top Issues: ${a.top_errors.join(', ')}`);
            lines.push('');
          }
          text = lines.join('\n');
        }
