...
import type { ErrorAnalysis, MessageRow } from '../types.js';
  ...
  - Returns empty calls array if no calls match filters`,
    ...
    async ({ agent, has_errors, analysis_status, limit, offset, response_format }) => {
      try {
        ...
        if (has_errors) query = query.gt('error_count', 0);
        ...
        const items = (calls ?? []).map((c) => ({
          ...
          agent: c.client_name as string,
          status: c.status as string,
          ended_reason: (c.ended_reason as string | null) ?? null,
          duration_seconds: (c.duration_seconds as number | null) ?? null,
          cost_usd: (c.cost_usd as number | null) ?? null,
          error_count: (c.error_count as number) ?? 0,
          critical_error_count: (c.critical_error_count as number) ?? 0,
          analysis_status: (c.analysis_status as string | null) ?? null,
        ...
        if (response_format === 'json') {
          ...
          if (hasMore) lines.push(`_Showing ${offset + 1}–${offset + items.length} of ${total} — use offset=${offset + limit} for next page_`);
          text = lines.join('\n');
        }
    ...
    async ({ call_id, include_transcript, response_format }) => {
      try {
        ...
        if (!call) {
          return {
            isError: true,
            content: [{ type: 'text' as const, text: `Error: Call not found: ${call_id}` }],
          };
        }

        const analysis = call.call_errors as ErrorAnalysis | null;
        const transcript = include_transcript
          ? (messages as MessageRow[] | null ?? []).map((m) => ({
        ...
        const result = {
          ...
          agent: call.client_name as string,
          status: call.status as string,
          ended_reason: (call.ended_reason as string | null) ?? null,
          duration_seconds: (call.duration_seconds as number | null) ?? null,
          cost_usd: (call.cost_usd as number | null) ?? null,
          created_at: call.created_at as string,
          ended_at: (call.ended_at as string | null) ?? null,
          analysis_status: (call.analysis_status as string) ?? 'pending',
          error_count: (call.error_count as number) ?? 0,
          critical_error_count: (call.critical_error_count as number) ?? 0,
          analysis: analysis ? {
  ...
  - Returns "Error: No messages found" if call has no transcript`,
    ...
    async ({ call_id, force, response_format }) => {
      try {
        ...
        if ((call.analysis_status as string) === 'complete' && !force) {
          return {
            content: [{ type: 'text' as const, text: 'Already analyzed. Pass force=true to re-analyze.' }],
          };
        }
        ...
        if (!messages || messages.length === 0) {
          return {
            isError: true,
            content: [{ type: 'text' as const, text: 'Error: No messages found for this call. Cannot analyze.' }],
          };
        }

        // Dynamic import to keep the MCP server decoupled from Next.js
        const { detectAgentType, analyzeCallErrors } = await import(
          '../../src/lib/error-analyzer.js' as string
        ) as {
          detectAgentType: (name: string) => string;
          analyzeCallErrors: (messages: Array<{ role: string; text: string; ordinal: number }>, agentType: string) => Promise<ErrorAnalysis>;
        };

        await db.from('ultravox_calls').update({ analysis_status: 'analyzing' }).eq('call_id', call_id);

        const agentType = detectAgentType(call.client_name as string);
        const analysis = await analyzeCallErrors(
          (messages as MessageRow[]).map((m) => ({ role: m.role, text: m.text, ordinal: m.ordinal })),
    ...
    async ({ response_format }) => {
      try {
        ...
        if (!baseUrl) {
          return {
            isError: true,
            content: [{ type: 'text' as const, text: 'Error: VOXRAY_URL env var not set. Set it to https://voxray.vercel.app or http://localhost:3000' }],
          };
        }
        ...
        if (!res.ok) {
          return {
            isError: true,
            content: [{ type: 'text' as const, text: `Error: Sync failed with status ${res.status}` }],
          };
        }

        const data = await res.json() as { synced: number; messages: number; analyzed: number; total: number };
