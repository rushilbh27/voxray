...
export function registerErrorTools(server) {
    server.registerTool('voxray_list_errors', {
  ...
  - Run \`npm run analyze\` in voxray project to trigger analysis`,
        ...
        try {
            ...
            for (const call of errorCalls ?? []) {
                const errors = call.call_errors?.errors ?? [];
                for (const err of errors) {
                    ...
                    if (err.severity === 'critical')
                        f.critical_count++;
                    if (!f.agents.includes(call.client_name))
                        f.agents.push(call.client_name);
                }
            }
            ...
            if (response_format === 'json') {
                ...
                if (hasMore)
                    lines.push(`_${allErrors.length - offset - limit} more error types — use offset=${offset + limit} to see next page_`);
                text = lines.join('\n');
            }
